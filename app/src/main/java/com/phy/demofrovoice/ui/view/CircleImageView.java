package com.phy.demofrovoice.ui.view;
...
/**
 * 自定义圆形imageView，scaleType 默认为CENTER_CROP，也只支持CENTER_CROP；
 */
...
public class CircleImageView extends AppCompatImageView {
    ...
    public CircleImageView(Context context, AttributeSet attrs, int defStyle) {
        ...
        if(null != attrs ){
            TypedArray ta = context.obtainStyledAttributes(attrs,R.styleable.CircleImageView,defStyle,0);
            ...
            //获取是否overLay
            mBorderOverlay = ta.getBoolean(R.styleable.CircleImageView_border_overlay,DEFAULT_BORDER_OVERLAY);
            //回收
            ta.recycle();
        }
        init();
    ...
    private void setup() {
        ...
        if (!mBorderOverlay) {
            //demo里始终执行
            //通过inset方法  使得图片显示的区域从mBorderRect大小上下左右内移边界的宽度形成区域，查看xml边界宽度为2dp（3px）,所以方形边长为就是160-girl4=156dp(234px)
